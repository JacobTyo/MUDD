...
if __name__ == "__main__":
    ...
    args.weights = PosixPath('../test/log/osnet_x1_0/model/model.pth.tar-2')
    ...
    onnx, openvino = flags  # export booleans
    
    im = torch.zeros(1, 3, args.imgsz[0], args.imgsz[1]).to('cpu')  # image size(1,3,640,480) BCHW iDetection
    if onnx:
        f = export_onnx(model, im, args.weights, 12, train=False, dynamic=args.dynamic, simplify=True)  # opset 12
    if openvino:
