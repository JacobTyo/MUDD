...
def main(num_clusters):
    ...
    with pymysql.connect(host=endpoint, user=username, password=password, database=database_name) as connection:
        with connection.cursor() as cursor:
            ...
            # sql = """SELECT id, embedding FROM detected_objects WHERE label_stage = %s"""
            # cursor.execute(sql, (1,))
            sql = """SELECT id, embedding FROM detected_objects"""
            cursor.execute(sql)
    ...
    # Update the detected_objects table
    with pymysql.connect(host=endpoint, user=username, password=password, database=database_name) as connection:
        with connection.cursor() as cursor:
            for obj_id, emb, cluster_label in id_embedding_cluster:
                is_group_center = obj_id == nearest_objects[cluster_label][0]
                sql = """
                    UPDATE detected_objects
                    SET group_id = %s, label_stage = %s, group_center = %s
                    WHERE id = %s
                """
                cursor.execute(sql, (cluster_label, 1, is_group_center, obj_id))
...
if __name__ == "__main__":
    ...
    argparse.add_argument('--num_clusters', type=int, default=1500)
